  ...
  return {
    visitor: {
      JSXElement(_path, state) {
        const propPathHandler = handleJSXPathProp(t, _path, state);
        if (Array.isArray(state.opts)) {
          for (const opts of state.opts) {
            const { componentName, propName, resolveFrom, targetPropName } = opts
            propPathHandler(componentName, propName, resolveFrom, targetPropName)
          }
        } else {
...
function handleJSXPathProp(t, _path, state) {
  return (componentName, propName, resolveFrom, targetPropName) => {
